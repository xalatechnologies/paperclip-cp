...
import {
  ...
  Shield, BookOpen, Heart, Target, ArrowUpRight, Terminal
} from 'lucide-react';
...
export function DashboardRealtime({
  safeCompanies,
  allAgents: staticAgents,
  agentLists,
  session,
  costSummary
}: any) {
  const liveAgentStatuses = useQuery(api.agents.listStatuses);
  const activeRuns = useQuery(api.routines.listActiveRuns) || [];

  // Merge live status into static agents
  const allAgents = staticAgents.map((agent: any) => {
    const live = liveAgentStatuses?.find((s: any) => s.paperclip_agent_id === agent.id);
    return live ? { ...agent, status: live.status } : agent;
  });

  const totalAgents  = allAgents.length;
  const errorAgents  = allAgents.filter((a: any) => a.status === 'error' || a.status === 'offline').length;
  const activeAgents = allAgents.filter((a: any) => a.status === 'active' || a.status === 'running').length;
  const idleAgents   = allAgents.filter((a: any) => a.status === 'idle' || a.status === 'online' || a.status === 'unknown').length;
      ...
      <PageBody>
        ...
        {errorAgents > 0 && (
          <AlertBanner
            ...
            detail={allAgents.filter((a: any) => a.status === 'error' || a.status === 'offline').map((a: any) => `${a.companyPrefix} → ${a.name}`).join(' · ')}
            href="/agents"
            variant="error"
          />
        )}
        ...
        {/* Stats */}
        <StatGrid cols={5}>
          <StatCard label="Companies"   value={safeCompanies.length} sub={`${safeCompanies.filter((c: any) => c.status === 'active').length} active`} icon={Building2} color="text-primary"     ring="primary"                                                                href="/companies" />
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden mb-6">
            <div className="px-5 py-3 border-b border-border bg-muted/20 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-semibold text-card-foreground">Live Output</span>
              </div>
              <span className="text-[11px] font-mono bg-primary/10 text-primary px-2 py-0.5 rounded-md">{activeRuns.length} running</span>
            </div>
            <div className="p-4 bg-background/50 max-h-64 overflow-y-auto font-mono text-[12px] leading-relaxed text-muted-foreground whitespace-pre-wrap">
              {activeRuns.map((run: any) => (
                <div key={run._id} className="mb-4 last:mb-0">
                  <div className="text-[10px] uppercase font-bold text-muted-foreground/60 mb-1 border-b border-border/50 pb-1">
                    Job: {run.vps_job_id} · Started: {new Date(run.started_at).toLocaleTimeString()}
                  </div>
                  <div className="text-foreground">
                    {run.live_output || run.output || 'Initializing run...'}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-3 space-y-6">
            <div>
              ...
              >
                {safeCompanies.map((company: any, i: number) => {
                  const agents = agentLists[i] ?? [];
                  const errs   = agents.filter((a: any) => a.status === 'error').length;
                  return (
                    <TR key={company.id}>
                      <TD sub={undefined}>
              ...
              {allAgents.length === 0 ? (
                ...
                <div className="bg-card border border-border rounded-xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 divide-x divide-border">
                    {allAgents
                      .sort((a: any, b: any) => (a.status === 'error' || a.status === 'offline') ? -1 : (b.status === 'error' || b.status === 'offline') ? 1 : 0)
                      .slice(0, 6)
                      .map((agent: any) => (
            ...
            {topAgents.length > 0 && (
              <div className="bg-card border border-border rounded-xl p-5">
                ...
                <div className="space-y-3">
                  {topAgents.slice(0, 4).map((a: any, idx: number) => {
